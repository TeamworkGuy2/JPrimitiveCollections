 ...
 * {@code Map<Integer, T>} by storing the Integers as type <code>int</code>
...
public class IntMapSorted<T> implements IntMapReadOnly<T>, RandomAccess {
	 ...
	 */
	public IntMapSorted(int capacity) {
	...
	@Override
	public IntMapSorted<T> copy() {
		IntMapSorted<T> newMap = new IntMapSorted<>(size);
		newMap.size = size;
	 ...
	 */
	@Override
	public int lastIndexOfKey(int key) {
		int idx = Arrays.binarySearch(keys, 0, size, key);
		return idx > -1 ? idx : -1;
	}
	 ...
	 * To determine the difference, call {@link #contains(int)}
	 */
	 ...
	 * To differentiate between a key with a null value and a new key, use {@link #contains(int)}
	 */
	public T put(int key, T value, boolean onlyIfAbsent) {
	...
	@SafeVarargs
	public final void putAll(Map.Entry<Integer, ? extends T>... entries) {
		if(size + entries.length > keys.length) {
			expandMap(size + entries.length);
		}

		for(Map.Entry<Integer, ? extends T> entry : entries) {
			put(entry.getKey(), entry.getValue());
		}
	}


	public final void putAll(Map<Integer, ? extends T> map) {
	...
	public final void putAll(Collection<? extends Integer> keys, Collection<? extends T> values) {
	...
	public final void putAll(Collection<? extends Integer> keys, Collection<? extends T> values,
		...
		Iterator<? extends Integer> keyIter = keys.iterator();
	...
	@SafeVarargs
	public static final <T> IntMapSorted<T> of(Map.Entry<Integer, T>... values) {
		IntMapSorted<T> inst = new IntMapSorted<>();
		inst.putAll(values);
		return inst;
	}
