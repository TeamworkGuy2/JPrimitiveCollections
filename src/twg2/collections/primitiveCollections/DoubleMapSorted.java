 ...
 * {@code Map<Double, T>} by storing the Doubles as type <code>double</code>
...
public class DoubleMapSorted<T> implements DoubleMapReadOnly<T>, RandomAccess {
	 ...
	 */
	public DoubleMapSorted(int capacity) {
	...
	@Override
	public DoubleMapSorted<T> copy() {
		DoubleMapSorted<T> newMap = new DoubleMapSorted<>(size);
		newMap.size = size;
	 ...
	 * To determine the difference, call {@link #contains(double)}
	 */
	 ...
	 * To differentiate between a key with a null value and a new key, use {@link #contains(double)}
	 */
	public T put(double key, T value, boolean onlyIfAbsent) {
	...
	@SafeVarargs
	public final void putAll(Map.Entry<Double, ? extends T>... entries) {
		if(size + entries.length > keys.length) {
			expandMap(size + entries.length);
		}

		for(Map.Entry<Double, ? extends T> entry : entries) {
			put(entry.getKey(), entry.getValue());
		}
	}


	public final void putAll(Map<Double, ? extends T> map) {
		for(Map.Entry<Double, ? extends T> entry : map.entrySet()) {
			put(entry.getKey(), entry.getValue());
		}
	}


	public final void putAll(Collection<? extends Double> keys, Collection<? extends T> values) {
	...
	public final void putAll(Collection<? extends Double> keys, Collection<? extends T> values,
		...
		Iterator<? extends Double> keyIter = keys.iterator();
	...
	@SafeVarargs
	public static final <T> DoubleMapSorted<T> of(Map.Entry<Double, T>... values) {
		DoubleMapSorted<T> inst = new DoubleMapSorted<>();
		inst.putAll(values);
		return inst;
	}
